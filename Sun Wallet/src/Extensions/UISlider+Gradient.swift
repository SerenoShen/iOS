...
extension UISlider {
    func addGradientTrack() {
        ...
        setMaximumTrackImage(imageForColors(colors: [(UIColor(named: "grayTextTint") ?? .white).cgColor, (UIColor(named: "grayTextTint") ?? .white).cgColor], offset: 4.0), for: .normal)
        setMinimumTrackImage(imageForColors(colors: [(UIColor(named: "gradientStart") ?? .white).cgColor, (UIColor(named: "gradientEnd") ?? .white).cgColor]), for: .normal)
    }
